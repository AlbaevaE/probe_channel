...
class LLM_Kyrgyz(Scene):
    def construct(self):
        ...
        name_lbls = VGroup(*[
            tx(n, size=0.30, color=GREY_2).next_to(layers[i], DOWN, buff=0.25)
        ...
        for i, (word, prob, col) in enumerate(candidates):
            ...
            wlbl = tx(word,                  size=0.38, color=col   ).next_to(bg, LEFT,  buff=0.20)
        ...
        chosen = tx("okuyм", size=0.52, color=TEAL).next_to(prompt_boxes, RIGHT, buff=0.3)
        ...
        summary = [
            ...
            ("Кийинки токенди болжолдойт",  TEAL),
